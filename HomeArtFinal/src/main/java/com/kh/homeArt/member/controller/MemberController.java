...
public class MemberController {
	...
	public String signUp(@ModelAttribute Member m, @RequestParam("profilePt") MultipartFile profilePt,
		...
		if (profilePt != null && !profilePt.isEmpty()) {
			...
			m.setRenameFileName(changeName);
	         ...
	         m.setOriginalFileName("basicMemberPhoto.png");
	         m.setRenameFileName("basicMemberPhoto.png");
	      }
	...
	public String findId(@ModelAttribute Member m) {
		...
		if (realMember != null) {
			// 메일전송
			String companyEmail = "itkw87@naver.com";
...
//이메일 인증
	...
	public String expertModify(@RequestParam("uploadFile") MultipartFile uploadFile,
			@RequestParam("address1") String address1, @RequestParam("address2") String address2, 
			@RequestParam("address3") String address3, @RequestParam("password1") String password1,
			@RequestParam("phoneNumber1") String phoneNumber1, @RequestParam("phoneNumber2") String phoneNumber2, 
			@RequestParam("phoneNumber3") String phoneNumber3, @RequestParam("email1") String email1, 
			@RequestParam("email2") String email2, @ModelAttribute Member modifyMember, 
			@RequestParam("workTime1") String workTime1, @RequestParam("workTime2") String workTime2,
			HttpServletRequest request, HttpSession session) {
		
		System.out.println("수정할 정보가 들어있는 멤버객체: " + modifyMember + "초기화 되어 있는 필드만 활용");
		...
		if (loginUser != null) {
			...
			loginUser.setNickName(modifyMember.getNickName());
			loginUser.setAddress(address1 + "/" + address2 + "/" + address3);
			loginUser.setPwd(encPwd);
			loginUser.setEmail(email1 + "@" + email2);
			loginUser.setPhone(phoneNumber1 + "-" + phoneNumber2 + "-" + phoneNumber3);
			loginUser.setField(modifyMember.getField());
			loginUser.setCareer(modifyMember.getCareer());
			loginUser.setWorkingDay(modifyMember.getWorkingDay());
